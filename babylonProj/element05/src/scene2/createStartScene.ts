...
function createElephants(scene: Scene) {
  const spriteManagerElephants = new SpriteManager(
    "elephantsManager",
    "./assets/sprites/elephant.png",
    2000,
  ...
  for (let i = 0; i < 500; i++) {
    const elephant: Sprite = new Sprite("elephant", spriteManagerElephants);
    elephant.position.x = Math.random() * -30;
    elephant.position.z = Math.random() * 20 + 8;
    elephant.position.y = 0.2;
  }

  for (let i = 0; i < 500; i++) {
    const elephant = new Sprite("elephant", spriteManagerElephants);
    elephant.position.x = Math.random() * 25 + 7;
    elephant.position.z = Math.random() * -35 + 8;
    elephant.position.y = 0.2;
  }
...
function createHemisphericLight(scene: Scene) {
  ...
  light.intensity = 0.5;
  light.diffuse = new Color3(255, 1, 1);
  light.specular = new Color3(255, 1, 1);
  light.groundColor = new Color3(255, 0.2, 1);
  return light;
}
